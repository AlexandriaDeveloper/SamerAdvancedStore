        ...
        public override void Up()
        {
            ...
            Sql(@"CREATE FUNCTION dbo.GetCurrentQuantity(@ProductId INT)
                ...
                DECLARE @QuantityMin INT;  
		     	 ...
		     	 SELECT  @QuantityPlus= SUM(ISNULL(ProductIn,0)) ,@QuantityMin=SUM(ISNULL(ProductOut,0))
			     FROM (SELECT ProductId,ProductIn,ProductOut FROM dbo.Transactions ) T
			     WHERE ProductId=@ProductId
		    	 group by ProductId
                 
               RETURN (ISNULL(@QuantityPlus,0)-ISNULL(@QuantityMin,0))
                End
           ...
           Sql(@"ALTER TABLE dbo.Transactions ADD Sold AS (ISNULL(round((ProductOut*PricePerUnit)-Discount,2) ,0));");
            ...
            SELECT @TotalOrder = sum(Sold) FROM dbo.Transactions  
